...
<!-- ... chaanging the backgroud color of button 
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        addToCartButtons.forEach(function(button) {
            button.addEventListener("click", function() {
                ...
                // Change the background color for the clicked state
                this.style.backgroundColor = "#000"; // Change to your desired color
...
<!--this section for displaying the books -->
 <section style="background-color: #eee;">
    <?php if (empty($books)): ?>
        <p>No books found for this subcategory.</p>
    <?php else: ?>
...
<form class="add-to-cart-form" action="<?= base_url('bookDetail/addToCart') ?>" method="post">
    ...
    <?php if (session()->has('user_id')): ?>
        ...
        <?php if ($is_in_cart[$book['b_id']]): ?>
            ...
            <?php if (!$is_rented[$book['b_id']]): ?>
                <!-- If the book is not rented -->
                <button type="submit" class="btn btn-primary btn-sm mt-2 w-100" name="action" value="remove">Remove from Cart</button>
                <!-- Display Rent Now button -->
                <button type="submit" class="btn btn-primary btn-sm mt-2 w-100" name="action" value="rent">Rent Now</button>
            <?php elseif (session()->has('rented_books') && in_array($book['b_id'], session()->get('rented_books'))): ?>
                <!-- If the book is rented for the current session -->
                <button type="submit" class="btn btn-primary btn-sm mt-2 w-100" disabled>Rented Now</button>
            <?php else: ?>
                <!-- If the book is rented in other sessions -->
                <button type="submit" class="btn btn-primary btn-sm mt-2 w-100" disabled>Already Rented</button>
            <?php endif; ?>
        <?php else: ?>
